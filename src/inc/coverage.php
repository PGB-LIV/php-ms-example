...
<p>Only sequences which have any coverage are shown. Uncovered sequences
    or sequences peptides referencing sequences not found in the FASTA
    file (i.e. generated decoys) will not be shown.</p>
<?php
    ...
    foreach ($fasta as $protein) {
        $proteins[$protein->getUniqueIdentifier()] = $protein->getSequence();
    }
    ...
    foreach ($mzident->getAnalysisData() as $spectra) {
        foreach ($spectra->getIdentifications() as $identification) {
            foreach ($identification->getPeptide()->getProteins() as $proteinEntry) {
                $acc = $proteinEntry->getProtein()->getAccession();
